...
class RobotDiferencial:
    ...
    def cinematica_directa(self, phi_dot_R, phi_dot_L):
        """
        Calcula la velocidad angular del robot (theta_dot) y el radio de giro (R)
        dadas las velocidades angulares de las ruedas.
        ...
        Returns:
            tuple: Una tupla que contiene (theta_dot, R).
                   theta_dot (float): Velocidad angular del robot (rad/s).
                   R (float): Radio de giro del robot (metros).
                   Retorna (theta_dot, None) si phi_dot_R == phi_dot_L.
        """
    ...
    def cinematica_inversa(self, x_dot, theta_dot):
        """
        Calcula las velocidades angulares de las ruedas (phi_dot_R, phi_dot_L)
        dadas la velocidad lineal y angular del robot.
        ...
        Returns:
            tuple: Una tupla que contiene (phi_dot_R, phi_dot_L).
                   phi_dot_R (float): Velocidad angular de la rueda derecha (rad/s).
                   phi_dot_L (float): Velocidad angular de la rueda izquierda (rad/s).
        """
    ...
    def velocidad_en_marco_local(self, phi_dot_R, phi_dot_L):
        """
        Calcula las velocidades del robot en su marco de referencia local.
        """
    ...
    def velocidad_en_marco_inercial(self, phi_dot_R, phi_dot_L, theta_actual):
        """
        Calcula las velocidades del robot en el marco de referencia inercial (global).
        """
    ...
    def simular_movimiento(self, punto_inicial, theta_inicial, duracion_total, velocidad_lineal, velocidad_angular):
        """
        Simula el movimiento del robot para un segmento dado.
        
        Args:
            punto_inicial (list): [x_inicial, y_inicial]
            theta_inicial (float): Orientación inicial en rad.
            duracion_total (float): Duración del segmento en segundos.
            velocidad_lineal (float): Velocidad lineal deseada para el segmento (m/s).
            velocidad_angular (float): Velocidad angular deseada para el segmento (rad/s).
        ...
        # Usar un número de pasos fijo para suavidad en la gráfica
        num_pasos = int(duracion_total * 100) # 100 pasos por segundo
...
def simular_trayectoria_completa(robot):
    print("\n======== TRAYECTORIA A SIMULAR Y CÁLCULOS DE COMANDOS ========")

    # Definiciones de los segmentos con duraciones ajustadas para respetar límites de motor
    segments_data = []

    # Segmento 1: Vuelta a la izquierda con radio de 0.5m y 180°
    radio_giro_1 = 0.5 # metros
    angulo_giro_1 = np.pi # 180 grados a la izquierda (positivo)
    duracion_giro_1 = 7.5 # segundos (Ajustado de 5.0s a 7.5s para respetar 50 RPM)
    theta_dot_giro_1 = angulo_giro_1 / duracion_giro_1
    x_dot_giro_1 = radio_giro_1 * theta_dot_giro_1
    ...
    # Segmento 3: Vuelta a la derecha radio de 0.5m y 180°
    radio_giro_2 = 0.5 # metros
    angulo_giro_2 = -np.pi # 180 grados a la derecha (negativo)
    duracion_giro_2 = 7.5 # segundos (Ajustado de 5.0s a 7.5s para respetar 50 RPM)
    theta_dot_giro_2 = angulo_giro_2 / duracion_giro_2
    x_dot_giro_2 = radio_giro_2 * abs(theta_dot_giro_2)
    ...
    # --- Impresión de cálculos de cada segmento y secuencia de comandos ---
    ...
    for i, seg in enumerate(segments_data):
        ...
        print(f"  Duración: {seg['duracion']:.2f} s")
        print(f"  Velocidad lineal deseada (robot): {seg['linear_x']:.4f} m/s")
        ...
        print(f"  cmd_vel_msg.angular.z = {seg['angular_z']:.4f}")
        print(f"  # Para publicar durante {seg['duracion']:.2f} segundos a 10 Hz:")
        print(f"  # for _ in range(int({seg['duracion']:.2f} * 10)):")
        print(f"  #     pub.publish(cmd_vel_msg)")
        print(f"  #     rate.sleep()")
        print("------------------------------------------------------------------")

    # --- Simulación y Recolección de Datos para la gráfica ---
    ...
    print(f"\nIniciando simulación de trayectoria desde: ({current_x:.2f}, {current_y:.2f}), Orientación: {current_theta:.2f} rad")
    ...
    for i, seg in enumerate(segments_data):
        ...
        # Almacenar la trayectoria del segmento (excluyendo el punto inicial duplicado)
        all_x.extend(seg_x[1:])
        all_y.extend(seg_y[1:])
        all_theta.extend(seg_theta[1:])

    print("\n======== SIMULACIÓN DE TRAYECTORIA COMPLETA FINALIZADA ========")
    ...
    temp_theta = 0.0
    for i, seg in enumerate(segments_data):
        _, _, _, next_point, _ = robot.simular_movimiento(
        ...
        # Actualizar temp_theta para la siguiente simulación del punto de transición
        temp_theta += seg['angular_z'] * seg['duracion']


    plt.xlabel('Posición X (metros)')
    plt.ylabel('Posición Y (metros)')
    plt.title('Trayectoria del Robot Diferencial (Ajustada a Límites de Motor)')
    plt.grid(True)
    plt.axis('equal') # Para asegurar que las escalas X e Y sean iguales
    plt.legend()
    plt.show()
...
if __name__ == "__main__":
    ...
    # Simular y graficar el camino descrito (Ejercicio 6)
    simular_trayectoria_completa(robot)

    # --- Cálculos Adicionales de Ejercicios Anteriores ---

    # 1. Trayectoria recta de 1[m] en 10 [s]
    print("\n======== CÁLCULOS ADICIONALES: TRAYECTORIA RECTA DE 1m EN 10s ========")
    ...
    print("\n======== CÁLCULOS ADICIONALES: TRAYECTORIA CIRCULAR (R=0.5m, HORARIO, 20s) ========")
    ...
    print("\n======== CÁLCULOS ADICIONALES: VELOCIDAD MÁXIMA (50 RPM) ========")
    max_motor_rpm = 50.0
