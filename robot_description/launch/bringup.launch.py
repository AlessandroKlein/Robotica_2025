...
def generate_launch_description():
    ...
    description_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare('robot_description_description'), '/launch/', 'description.launch.py'  # Incluye la descripción del robot
        ]),
    ...
    gazebo_launch = IncludeLaunchDescription(
        PythonLaunchDescriptionSource([
            FindPackageShare('robot_description_gz'), '/launch/', 'gazebo.launch.py'  # Incluye el lanzamiento de Gazebo
        ])
    )
    ...
    controller_manager = Node(
        ...
        parameters=[PathJoinSubstitution([FindPackageShare('robot_description_control'), 'config', 'diffbot_controllers.yaml'])],  # Configuración de controladores
        output='screen'
    )
    ...
    # Nodo de odometría
    odometry_node = Node(
        package='robot_description',  # Nodo de odometría personalizado
    ...
    # RViz
    rviz_config = PathJoinSubstitution([FindPackageShare('robot_description'), 'diffbot.rviz'])  # Ruta al archivo de configuración de RViz
    rviz_node = Node(
